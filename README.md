...
- Output renaming for all textures, and normal-map generation for materials without one — either a **blank** map or a real one **generated from the diffuse**. The generator is a numpy port of the gimp-normalmap plug-in workflow these unit textures were made with (4 sample filter, scale 4, min Z 0, invert X and Y), so the export needs neither GIMP nor the plug-in nor any external process. **Scale** sets how deep the detail reads in game; the generated **alpha is the specular map**, so **Brightness** and **Contrast** — on GIMP’s own -127..127 scale — are what decide how shiny the unit comes out. The two toggles are mutually exclusive: they fill the same slot, so switching one on switches the other off
