...
def IKGenerator(name, x, y, z):
    ...
    arm.collections['Med2'].assign(bone)
    bone = arm.edit_bones.new('IK Bow String')
    bone.head = -0.5303, -0.0116, 0.4902
    bone.tail = -0.5303, 0.2971, 0.4902
    bone.roll = 0.0000
    bone.use_connect = False
    bone.parent = arm.edit_bones[bones['IK frist  left']]
    bones['IK Bow String'] = bone.name
    arm.collections['IK Control Bones'].assign(bone)
    ...
    pbone = obj_arm.pose.bones['IK Bow String']
    cons = pbone.constraints.new('COPY_LOCATION')
    cons.target = bpy.data.objects[obj_arm.name]
    cons.subtarget = "bone_sling"
    fcurve = obj_arm.driver_add('pose.bones["IK Bow String"].constraints["Copy Location"].influence')
    drv = fcurve.driver
    drv.type = 'SCRIPTED'
